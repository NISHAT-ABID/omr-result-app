...
def validate_omr_image(image_bgr):
    ...
    if w < MIN_WIDTH or h < MIN_HEIGHT:
        errors.append(f"Image resolution is too low ({w}x{h}). Please retake with higher resolution.")
    gray = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY)
    mean = float(np.mean(gray))
    if mean < DARK_MEAN_THRESHOLD:
        errors.append("The photo is too dark. Please retake in better lighting.")
    elif mean > BRIGHT_MEAN_THRESHOLD:
        warnings.append("The photo looks overexposed.")
    blur = float(cv2.Laplacian(gray, cv2.CV_64F).var())
    if blur < BLUR_VARIANCE_THRESHOLD:
        errors.append("The photo looks blurry. Please hold the camera steady.")
...
def detect_and_warp(image_bgr):
    ...
    return cv2.warpPerspective(orig, matrix, (WARP_WIDTH, WARP_HEIGHT)), True
...
def _refine_bubble_center(image_bgr, center, search=32):
    ...
    if crop.size == 0: return np.asarray(center, dtype=np.float32), 0.0
    circles = cv2.HoughCircles(crop, cv2.HOUGH_GRADIENT, dp=1.0, minDist=10, param1=70, param2=10, minRadius=7, maxRadius=24)
    if circles is None: return np.asarray(center, dtype=np.float32), 0.0
    best = None
    for cx, cy, r in circles[0]:
        px, py = x0 + float(cx), y0 + float(cy)
        dist = float(np.hypot(px - x, py - y))
        if dist > search * 0.9: continue
        score = (search - dist) + min(float(r), 20.0) * 0.15
        if best is None or score > best[0]: best = (score, px, py)
    if best is None: return np.asarray(center, dtype=np.float32), 0.0
...
def align_to_master_grid(image_bgr, grid, min_matches=30):
    ...
    for q_no, options in grid.items():
        ...
        for opt in OPTIONS:
            ...
            if confidence > 0: moved.append(float(np.linalg.norm(point - original)))
    ...
    return image_bgr, {"applied": True, "matches": len(moved), "inliers": len(moved), "average_center_adjustment_px": round(avg_move, 2), "grid": refined}


def preprocess_omr_image(image_bgr):
    """Prepare raw OMR image for bubble metrics (NOT binary)."""
    if image_bgr is None or image_bgr.size == 0:
        raise ValueError("Empty OMR image.")
    # Use max channel to avoid pink print looking dark
...
def _bubble_metrics(image_red, center, radius):
    ...
    if patch.size == 0: return 0.0, 0.0, 255.0, 255.0
    mh, mw = patch.shape[:2]
    cm = core_mask[:mh, :mw]
    rm = ring_mask[:mh, :mw]
    core = patch[cm]
    ring = patch[rm]
    if core.size == 0: return 0.0, 0.0, 255.0, 255.0
...
def read_answers(warped_bgr, grid, dark_threshold=DARK_PIXEL_THRESHOLD, min_gap=15, radius=None):
    ...
    answers = {}
    for q_no, options in grid.items():
        metrics = {opt: _bubble_metrics(red, center, radius) for opt, center in options.items()}
        ...
        is_multi = (best_contrast >= MULTI_MIN_CONTRAST and second_contrast >= MULTI_MIN_CONTRAST and inks[best] >= MULTI_MIN_INK_FRACTION and inks[second] >= MULTI_MIN_INK_FRACTION and strength2 >= MULTI_RATIO_LIMIT)
        if is_multi:
            answers[q_no] = "MULTI"
            continue
...
def score_answers(student_answers, key_string, negative_marking=False, negative_value=0.0):
    ...
    for i in range(total):
        ...
        if given == "MULTI":
            wrong.append(q)
            wrong_details[q] = {"given": "Multiple", "correct": correct_ans}
        elif given == correct_ans:
            correct += 1
        else:
            wrong.append(q)
            wrong_details[q] = {"given": given, "correct": correct_ans}
